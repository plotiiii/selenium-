...
from wanwoo.common.handle_path import ERROR_IMG
...
class BasePage:
    ...
    def wait_element_visibility(self, locator, img_info, timeout=15, poll_frequency=0.5):
        ...
        try:
            ...
            # 输出日志
            log.error("元素--{}--等待可见超时".format(img_info))
            log.exception(e)
            # 对当前页面进行截图
    ...
    def wait_element_presence(self, locator, img_info, timeout=15, poll_frequency=0.5):
        ...
        try:
            ...
            # 输出日志
            log.error("元素--{}--等待被加载超时".format(img_info))
            log.exception(e)
            # 对当前页面进行截图
            self.save_scree_image(img_info)
            raise e
    ...
    def get_element(self, locator, img_info):
        ...
        try:
            ...
            # 输出日志
            log.error("获取元素--{}--失败".format(img_info))
            log.exception(e)
            # 对当前页面进行截图
            self.save_scree_image(img_info)
            raise e
